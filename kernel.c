...
/* passing the address of the multiboot info boot.S */
void entry(unsigned long magic, unsigned long addr) {
    ...
    fs_init(fs_start);
		//baby test
		/* fs_dir_open((const uint8_t*)".");
		uint8_t buf[32];
		fs_dir_read(0, buf, 32);
		printf("FILE READ TEST\n");
		fs_file_open((const uint8_t*)"verylargetextwithverylongname.tx");
		uint8_t buff[9999];
		fs_file_read(0,buff,9999);
		printf("\nFilesystem test\n");
		printf("%s\n",buf);
		printf("%s\n",buff); */
		//printf("\n");
    ...
    // 8 is the IRQ line number for the RTC (Slave PIC IRQ0)
    rtc_open(NULL);
    
    // 1 is the IRQ line number for the keyboard (Master PIC IRQ1)
   // kbd_init(1);
    //printf("EOI sent\n");
    init_pit();
